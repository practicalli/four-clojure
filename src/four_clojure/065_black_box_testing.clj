...
(defn order [order-id date amount]
...
(defn what-sequence-is-it-range-hack [xs]
  (cond
    (map?  xs)   :map
    (vector? xs) :vector
    (set? xs)    :set
...
(defn what-sequence-no-restricted [xs]
  (cond
...
(defn what-sequence-no-restricted-coll [xs]
  (cond
    (coll? (first xs)) :map
    (vector? xs)       :vector
    (set? xs)          :set
...
(defn what-sequence-no-restricted-reverse [xs]
  (cond
    (coll? (first xs))  :map
    (vector? xs)        :vector
    (= xs (reverse xs)) :set
...
(defn what-sequence-no-restricted-reverse-let [xs]
  (cond
    (coll? (first xs))                  :map
    (vector? xs)                        :vector
...
(defn what-sequence-no-restricted-peek [xs]
  (cond
    (coll? (first xs))     :map
...
(defn what-sequence-no-restricted-conj [xs]
  (cond
    (coll? (first xs))        :map
...
#_{\( :list \{ :map \[ :vector \{ :map }



(fn [coll]
  (get {\# :set \{ :map \[ :vector \c :list}
       (first (str coll)))
  )
